...
gw_info_t *get_old_gateway_frame_alloc(uint8 *buffer, int length)
{
	...
	gw_info_t *ogw_info = osal_mem_alloc(sizeof(gw_info_t));
	memset(ogw_info, 0, sizeof(gw_info_t));
...
{	
	...
	gw_info_t *gw_info = osal_mem_alloc(sizeof(gw_info_t));
	memset(gw_info, 0, sizeof(gw_info_t));
...
void analysis_zdev_frame(frhandler_arg_t *arg)
{
	...
	case HEAD_UH:
	{
		...
		if(dev_info == NULL)
		{
			...
			ConnectorApp_TxHandler((uint8 *)mbuf, 14);
		}
		else
		{
